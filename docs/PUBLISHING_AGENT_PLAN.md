...
| 2 | YouTube-Upload über offizielle API (Privacy-Status wählbar, `publishAt`) — **Dry-Run (Phase 1) + OAuth-Readiness & keyring-Token-Ablage (Phase 2) bereits gebaut**, siehe [`YOUTUBE_PUBLISHING.md`](YOUTUBE_PUBLISHING.md). Echter Upload bleibt `not_implemented`. | interaktiver OAuth-Flow + Upload-Client + ggf. API-Audit |
