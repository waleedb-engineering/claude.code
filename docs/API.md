...
| `POST …/youtube/publish` | **Echter PRIVATER Upload** (Phase 3, mit Retry/Backoff). Body `{confirm:"UPLOAD_PRIVATE", privacy_status:"private"}`. Immer **HTTP 200** mit `success`, `error_code`, `status`, `external_post_id`, `privacy_status:"private"`, `idempotency_state`, `retry_count`, `published_at`, `message`, `no_secrets:true`. `published`/`external_post_id` **nur** bei eindeutigem API-Erfolg; `public`/`unlisted` → `invalid_privacy_status`; Flag aus → `upload_disabled`; fehlt `google-api-python-client` → `upload_dependency_missing`. Retriable `5xx`/`429`/`rateLimitExceeded`/Netzwerk → Backoff auf **derselben** resumable Session; `quotaExceeded`/permission **nicht** retriable; `401` → **ein** Refresh; Retries erschöpft/Netzabbruch → `upload_result_uncertain`. **Nie Token/Secrets.** |
| `GET …/youtube/upload-status` | Status/Recovery: `state` (State-Machine), `is_stale`, `idempotency_state`, `publish_attempt_count`, `current_attempt`, `retry_count`, `last_publish_error`, `last_error_category`, `external_post_id_present`, `can_retry`, `can_reconcile`, `requires_manual_check`, `requires_reauth`, `last_activity_at`, `reconciliation_status`, `upload_progress`, `attempt_history_summary`, `transition_history_summary`, `no_secrets:true`. Löst nichts aus. **Nie Token/Secrets/Session-URIs.** |
| `POST …/youtube/reconcile` | Prüft NUR den Remote-Status einer bekannten `external_post_id` (**exakte ID, keine Heuristik**) → `published`/`failed`/`uncertain`. **Startet NIE einen Upload.** Antwort = aktualisierter upload-status. |
